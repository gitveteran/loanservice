...
@SpringBootTest
class CreditLoanClientTest {

    @MockBean
    private RestTemplate mockRestTemplate; // Mocked RestTemplate bean

    @Autowired
    private CreditLoanClient creditLoanClient; // Spring-managed CreditLoanClient bean
    ...
    void testCreateLoan_Success() {
        ...
        // Call the method
        String response = creditLoanClient.createLoan(loanRequest);
    ...
    void testCreateLoan_Fallback() {
        ...
        Exception exception = assertThrows(Exception.class, () -> {
            creditLoanClient.createLoan(loanRequest);
        });

        // Verify the exception message
        assertEquals("Fallback Loan ID due to: Credit Service unavailable", exception.getMessage());
