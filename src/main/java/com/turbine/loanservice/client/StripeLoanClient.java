...
@Component("stripe")
public class StripeLoanClient implements ThirdPartyLoanClient {

    private final RestTemplate thirdpartyLoanApiClient;

    @Value("${loanservice.thirdparty.stripe.url}")
    private String loanUrl;

    @Autowired
    public StripeLoanClient(RestTemplate thirdpartyLoanApiClient) {
    ...
    public String fallbackCreateLoan(Loan loanRequest, Exception ex) throws Exception {
        throw new RuntimeException("Fallback Loan ID due to: Stripe Service unavailable");
    }
}
